...
class CriticNetwork:
    ...
    def __init__(self, sess, time_steps, state_dim, c_dim, action_dim=1):
        ...
        self.action_embed_dim = c_dim
        self.hidden_units = 64
        self.lstm_hidden_units = config.CriticNetwork_LSTM_LSTM_HIDDEN_UINITS
        self.lstm_layer_num = config.CriticNetwork_StackedLSTM_LSTM_LAYER_NUM
    ...
    def create_network(self):
        ...
        with tf.name_scope('critic_network'):
            ...
            Waction = tf.Variable(tf.random_uniform([self.action_dim, self.action_embed_dim], -3e-3, 3e-3), name = 'action_weight')
            baction = tf.Variable(tf.random_uniform([self.action_embed_dim], -3e-3, 3e-3), name = 'action_bias')

            W1 = tf.Variable(tf.random_uniform([self.lstm_hidden_units + self.action_embed_dim + self.c_dim, self.hidden_units], -3e-3, 3e-3),
                             name='weight')
            b1 = tf.Variable(tf.random_uniform([self.hidden_units], -3e-3, 3e-3), name='bias')

            W2 = tf.Variable(tf.random_uniform([self.hidden_units, 1], -3e-3, 3e-3), name = 'weight')
            b2 = tf.Variable(tf.random_uniform([1], -3e-3, 3e-3), name = 'bias')

            action_embed = tf.matmul(action_input, Waction) + baction

            lstm_cells = [rnn_cell.BasicLSTMCell(self.lstm_hidden_units, forget_bias = 0.6, reuse = tf.AUTO_REUSE) for _ in range(self.lstm_layer_num)]
            mul_cells = rnn_cell.MultiRNNCell(lstm_cells)
            #lstm_cell = rnn_cell.BasicLSTMCell(self.lstm_hidden_units, forget_bias=0.8, reuse=tf.AUTO_REUSE,
                                               #name='LSTM')
            outputs, states = rnn.static_rnn(mul_cells, state_lstm_input, dtype=tf.float32)
            output_action_c = tf.concat([outputs[-1], action_embed, c_input], axis=1)
            output_hidden = tf.matmul(output_action_c, W1) + b1
            output = tf.matmul(output_hidden, W2) + b2
            #net = [Waction, baction, W1, b1, W2, b2] + lstm_cell.weights
            net = [Waction, baction, W1, b1, W2, b2]
    ...
    def create_target_network(self, net):
        ...
        with tf.name_scope('target_critic_network'):
            ...
            action_embed = tf.matmul(action_input, target_net[0]) + target_net[1]
            ...
            outputs, states = rnn.static_rnn(mul_cells, state_lstm_input, dtype=tf.float32)
            output_action_c = tf.concat([outputs[-1], action_embed, c_input], axis=1)
            output_hidden = tf.matmul(output_action_c, target_net[2]) + target_net[3]
            q_value_output = tf.matmul(output_hidden, target_net[4]) + target_net[5]

            return state_input, c_input, action_input, q_value_output, target_update, mul_cells, target_net
    ...
    def target_q(self, state_batch, c_batch, action_batch):
        ...
        self.lstm_cell.set_weights(net[6:])
    ...
    def save_weights(self, path):
        self.Saver.save(self.sess, path + 'critic-network')

    def load_weights(self, path):
        self.saver = tf.train.import_meta_graph(path + 'critic-network.meta')
